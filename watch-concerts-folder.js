...
const concertsDir = join(__dirname, 'public', 'images', 'Concerts');
...
// Get all files from Concerts folder
async function getConcertsFiles() {
  try {
    const files = await readdir(concertsDir);
    const filePaths = [];
    const processedBaseNames = new Set(); // Track base names to avoid HEIC/JPG duplicates
    
    for (const file of files) {
      const filePath = join(concertsDir, file);
    ...
    console.error('Error reading Concerts folder:', error);
    return [];
  }
}
...
async function updateDesktopData() {
  try {
    const concertsFiles = await getConcertsFiles();
    ...
    // Find the badbo_desktop image
    const badboImage = desktopData.images.find(img => img.id === 'badbo_desktop');
    
    if (badboImage) {
      // Update the imageStack with all files from Concerts folder
      badboImage.imageStack = concertsFiles;
      
      // Write back to file
      await writeFile(desktopDataPath, JSON.stringify(desktopData, null, 2) + '\n', 'utf-8');
      
      console.log(`✅ Updated imageStack with ${concertsFiles.length} files from Concerts folder`);
      console.log(`📁 Files: ${concertsFiles.map(f => basename(f)).join(', ')}`);
    } else {
      console.warn('⚠️  badbo_desktop image not found in desktopData.json');
    }
  } catch (error) {
...
function startWatching() {
  console.log(`👀 Watching ${concertsDir} for changes...\n`);
  
  // Initial update
  updateDesktopData();
  
  // Watch for file changes
  watch(concertsDir, { recursive: false }, async (eventType, filename) => {
    if (filename) {
      console.log(`\n📝 Detected change: ${filename} (${eventType})`);
      
      // Small delay to ensure file is fully written
      setTimeout(async () => {
        const filePath = join(concertsDir, filename);
