...
const niagarasDir = join(__dirname, 'public', 'images', 'Niagaras');
...
// Get all files from Niagaras folder
async function getNiagarasFiles() {
  try {
    const files = await readdir(niagarasDir);
    const filePaths = [];
    const processedBaseNames = new Set(); // Track base names to avoid HEIC/JPG duplicates
    
    for (const file of files) {
      const filePath = join(niagarasDir, file);
    ...
    console.error('Error reading Niagaras folder:', error);
    return [];
  }
}
...
async function updateDesktopData() {
  try {
    const niagarasFiles = await getNiagarasFiles();
    ...
    // Find the fam_desktop image
    const famImage = desktopData.images.find(img => img.id === 'fam_desktop');
    
    if (famImage) {
      // Update the imageStack with all files from Niagaras folder
      famImage.imageStack = niagarasFiles;
      
      // Write back to file
      await writeFile(desktopDataPath, JSON.stringify(desktopData, null, 2) + '\n', 'utf-8');
      
      console.log(`✅ Updated imageStack with ${niagarasFiles.length} files from Niagaras folder`);
      console.log(`📁 Files: ${niagarasFiles.map(f => basename(f)).join(', ')}`);
    } else {
      console.warn('⚠️  fam_desktop image not found in desktopData.json');
    }
  } catch (error) {
...
function startWatching() {
  console.log(`👀 Watching ${niagarasDir} for changes...\n`);
  
  // Initial update
  updateDesktopData();
  
  // Watch for file changes
  watch(niagarasDir, { recursive: false }, async (eventType, filename) => {
    if (filename) {
      console.log(`\n📝 Detected change: ${filename} (${eventType})`);
      
      // Small delay to ensure file is fully written
      setTimeout(async () => {
        const filePath = join(niagarasDir, filename);
